...
public class Q4 {
	...
	public static void main(String[] args) {
		String content = "public class HelloWorld { \n" +
				...
				" }\n" +
				"}\n";
		String text = readFile("src/Q4Example");
		scan(content);
	...
	public static void scan(String prog) {
		...
		while (index < n) {
			...
			// than integer
				...
				while (index < n) { // collects all digits in the string
					...
					if (!isDigit(ch)) { //while the character is not a digit continue to traverse the input
				...
				if (word.charAt(1) == '.') { //if the created word (number) contains a period
			...
			// the string of characters
				...
				System.out.println("Error: wrong identifier name.");
			}
		}
		...
		/**
		 * This program will take variable names
		 * If the name contains anything other than letters, digits or underscores it is invalid
		 * The name must start with a letter or underscore to be valid. 
		 * If the variable name is invalid the user will be told 
		 * the input is invalid and to try again
		 * If the value is valid then it will be printed out.
		 * @param args
		 */

		public static boolean variableNameCheck(String varName) {
			boolean valid = false; //boolean values are really useful for checking state within a program i.e. valid user input
			String variableName = "";
			Scanner in = new Scanner(System.in);
			/*
			 * While the boolean variable valid is set to false continue to execute the statements in the while loop
			 */
			
			while (!valid) {
				System.out.print("Please enter a variable name: ");
				variableName = in.nextLine(); // read in a variable name
				char ch = variableName.charAt(0);
				if ((Character.isLowerCase(ch) || variableName.charAt(0) == '_') && (variableName.contains(" ") == false && variableName.contains("+") == false)) { //check if that value starts with a letter or '_'
					for (int i = 1; i < variableName.length(); i++) {
						char c = variableName.charAt(i);	
						if (Character.isAlphabetic(c) || Character.isDigit(c) || c == '_') 
						// check if that value contains anything other than letters, digits or underscores		
						valid = true;
					}	//if the number entered is valid then set the boolean variable to true i.e. the while loop will stop execution
					System.out.println("Valid input of " + variableName); //output the results
				} else { 
					System.out.println("Invalid input. Valid variable names must start with a lowercase letter or an underscore ('_') and must only contain letters, digits or underscores ('_')."); //if the input is invalid then tell your user 
				}
			}
