...
public class Q3 {
	...
	public static void main(String[] args) {
		String content = "public class HelloWorld { \n" +
				...
				" }\n" +
				"}\n";
		String text = readFile("src/Q3Example");
		scan(content);
	...
	public static void scan(String prog) {
		...
		while (index < n) {
		...
		}else if (ch == '/' && prog.charAt(index+1) == '/') { //identify single line comments
			index++;
			index++;
			while (ch != '\r' || ch != '\n') {
				index++;
			...
			else if (ch == '/' && prog.charAt(index+1) == '*') { //identify double line comments
				index++;
				index++;
				while (index < n) {
					if (ch != '*' && prog.charAt(index+1) != '/') {
					index++;
					continue;
				}else {
				index++;
